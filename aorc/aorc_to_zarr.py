...
output_path = pathlib.Path(
    "/glade/p/datashare/jamesmcc/aorc")
...
file_chunked = output_path / "aorc.zarr"
file_step = output_path / "step.zarr"
file_last_step = output_path / "last_step.zarr"
file_temp = output_path / "temp.zarr"
file_log_loop_time = output_path / "aorc_loop_time.txt"
file_lock = output_path / "aorc_write_in_progress.lock"
...
def preprocess_aorc(ds):
    drop_vars_full = [
        "reference_time", "crs", 'U2D',
        'V2D', 'LWDOWN',
        # 'T2D',  # aorc
        # 'Q2D',  # surface_specific_humidity
        # 'PSFC',  # pressure
        # 'SWDOWN',  # short wave solar
        'RAINRATE',  # already processed precip previously
        'LQFRAC', 'x', 'y']
    drop_vars = list(
